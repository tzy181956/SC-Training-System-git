# CHANGELOG

## Unreleased / V1-dev

### Added

- 增加训练模式与训练 session 主流程
- 增加计划分配概览、分组展示、批量取消分配
- 增加训练模式下当前组提交、历史组即时修改
- 增加训练状态颜色提示、自动完成动作、自动完成 session
- 增加跨电脑更稳的 Windows 启动 / 初始化脚本逻辑
- 增加运动员主档身体字段：
  - `height`
  - `weight`
  - `body_fat_percentage`
  - `wingspan`
  - `standing_reach`
- 增加测试记录展示字段：
  - `result_text`
- 增加真实 Excel 数据导入脚本：
  - [import_real_test_data.py](C:\Users\Tian Ziyu\Documents\New project\backend\scripts\import_real_test_data.py)
- 增加运行时 schema 同步：
  - [schema_sync.py](C:\Users\Tian Ziyu\Documents\New project\backend\app\core\schema_sync.py)
- 增加真实数据作为默认长期数据路径，初始化后可单独执行导入脚本恢复数据
- 增加开发侧危险数据脚本的二次确认能力，避免误清理真实数据

### Changed

- 训练模式改为“点计划即训练”，不再依赖开始训练前的确认页
- 当前组恢复为手动确认提交，历史组保持即时修改
- 训练录入区改成更适合 iPad 的大按钮交互
- 训练模式左侧队员计划改为常驻展开
- 训练模式侧边栏新增日期旁的队伍筛选
- 测试数据页改为支持展示时间文本和力量体重比计算
- 运动员页扩展为可编辑身体主档数据
- 缺少测试基准时，计划分配不再被阻断，改为“训练时录入”
- 初始化脚本改为只创建/补齐数据库结构，不再删库、不再自动灌入 demo 数据
- 启动脚本的自动初始化链路不再导致真实数据被覆盖
- 真实数据导入脚本改为先要求输入固定确认词，再执行批量清理和导入

### Fixed

- 修复跨电脑复制 `.venv` 后因旧解释器绝对路径导致初始化失败的问题
- 修复 Node `spawn EPERM` 排查后造成的误判，明确区分环境限制与项目问题
- 修复模板页面“新增动作卡片看起来可点但实际无反应”的交互问题
- 修复计划分配概览接口被动态路由误拦截的问题
- 修复训练最后一组提交后动作状态不同步的问题
- 修复历史脏 session 中已满组但仍显示 `in_progress` 的问题
- 修复训练录入中默认伪造 `20` 公斤的问题
- 修复真实 Excel 导入时把“全队平均”汇总行误导入为运动员的问题
- 修复管理模式与训练模式中多处侧栏/列表区域无限向下增长的问题，改为容器内滚动
- 修复外层壳层高度未被真正限制导致内部滚动未生效的问题
- 修复动作库与运动员列表中部分中文名称/描述在滚动容器内仍然出框的问题
- 调整动作库与运动员左侧列表卡片的自适应宽度和文本层级，避免中文被硬切后难以阅读
- 调整运动员左侧列表卡片的高度与三行内容布局，避免第二行与身高体重信息被卡片高度裁掉
- 统一前端多处列表/卡片的自适应文本规则，覆盖动作库、运动员、测试数据、训练模式侧栏、模板动作卡片和训练报告卡片，减少长中文标题和多行副文本导致的遮挡与硬切
- 修正测试数据页折线图的取数逻辑，改为只展示当前运动员在当前测试项目下的历史趋势，不再把不同测试项目混在同一条折线里
- 为测试数据页折线图增加当前测试项目的全队平均趋势线，并将图表标题改为“运动员 + 测试项目趋势”
- 将测试数据页中的全队趋势升级为“最高/最低区间阴影 + 全队平均线”，用于直观看到个人成绩与队伍波动范围的关系
- 重构测试数据页区间带实现方式，增加清晰可见的最高/最低边界线、稳定的浅色区间阴影，以及显示个人/平均/最高/最低的 tooltip
- 将测试数据页区间图进一步调整为“最高点线 + 最低点线 + 更高透明度区间填充”，避免全队区间在页面上难以辨认
- 为测试数据页补充单日期点场景下的窄竖向区间带，避免只有一个测试日期时最高/最低点可见但区间阴影完全不显示

### Notes

- 当前仓库已从“纯 demo 演示”过渡到“真实女篮青年队测试数据验证”
- 本地数据库当前状态：
  - 16 名真实队员
  - 368 条测试记录
  - 训练模板与动作库保留
  - 示例运动员、测试记录、计划分配、训练记录已清空
- 目前仍没有正式数据库迁移框架，模型扩展依赖：
  - `backend/scripts/init_db.py`
  - `backend/app/core/schema_sync.py`
- demo 初始化入口已移除，真实数据恢复/导入入口为：
  - `backend/scripts/import_real_test_data.py`
