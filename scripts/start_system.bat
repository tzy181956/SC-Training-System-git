...
if not exist "%POWERSHELL_EXE%" (
  ...
  >"%SUMMARY_FILE%" (
    ...
    echo Suggested fix: Restore Windows PowerShell access, then rerun start_system.bat.
...
"%POWERSHELL_EXE%" -NoProfile -ExecutionPolicy Bypass -File "%~dp0system_launcher.ps1" -Mode start
exit /b %errorlevel%
