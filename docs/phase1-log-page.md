...
> 文档状态：历史阶段文档  
> 当前用途：记录日志页第一版接入背景。  
> 注意：文中若出现过时的权限或运行形态表述，以当前代码和根目录权威文档为准，当前系统实际为登录制。
...
从当前实现回看，这一节对应的历史背景应理解为：

- 当时日志页先补的是按 `team_id` 过滤所需的数据结构
- 当前系统实际已经进入登录制，`admin / coach` 权限链路已接入
- 因此本段只保留“为什么当时先补字段”的背景，不再代表当前默认可见范围
