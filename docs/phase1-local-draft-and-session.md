...
- 复杂冲突合并界面
- 更细的 `syncing / sync_error / 待处理` 生命周期状态
- 独立的同步冲突处理后台页面

也就是说，当前实现已经从“最小增量补传”推进到了“增量补传 + 整课覆盖兜底 + 最小冲突可见性”阶段，但还没有进入“复杂冲突处理界面”和“更完整同步状态模型”阶段。
...
`sync_error` 仍属于后续阶段增强，不在本轮实现范围内。
...
|---|---|---|
| 已命中计划但尚未正式开始 | `planned` | 无正式 session 或未确认 |
| 第一组已提交 | `in_progress` | `in_progress` |
| 全部完成 | `completed` | `completed` |
| 到自动收口时一组未做 | `absent` | `absent` |
| 到自动收口时部分完成 | `partial_complete` | `partial_complete` |

### 关键原则

- `planned` 可以存在于 local draft / 训练执行状态层
- `planned` 不应成为正式 `session` 的长期对外状态
...
## 8. 第一阶段建议新增的表或字段（只提设计，不落迁移）

这一步只提设计建议，不落地数据库迁移。

## 8.1 新增表建议：`session_local_drafts`
...
1. 本地草稿层
2. `session_key`
3. 最小 `sync_status`（`synced / pending`）
4. `pending_operations` 单组补传队列
5. “第一组成功后才正式进入 `in_progress`” 的状态切换机制

当前仍保留的阶段性差距是：

1. `syncing / sync_error` 以及长时间失败后的“待处理”机制还没有落地
2. 教练端 / 管理端统一可见的同步状态面板还没有落地
3. 当前只有最小冲突提示和日志，还没有独立冲突处理界面
