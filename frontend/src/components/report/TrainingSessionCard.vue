...
defineProps<{ session: any; onlyIncomplete?: boolean; onlyMainLift?: boolean }>()
      ...
      >
        <div class="item-head">
          <div class="item-copy adaptive-card">
            ...
            <p class="adaptive-card-subtitle adaptive-card-clamp-2">{{ item.completed_sets }}/{{ item.prescribed_sets }} 组，目标 {{ item.prescribed_reps }} 次</p>
          </div>
          <span class="item-badge" :class="{ main: item.is_main_lift }">{{ item.is_main_lift ? '主项动作' : '常规动作' }}</span>
        </div>
        ...
        <div class="record-table-wrap">
          <table class="record-table">
            <thead>
              <tr>
                <th>组次</th><th>目标重量</th><th>实际重量</th><th>次数</th><th>RIR</th><th>建议重量</th><th>最终采用</th><th>调整说明</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="record in item.records" :key="record.id">
                ...
                <td><span class="decision-pill" :class="adjustmentClass(record.adjustment_type)">{{ record.adjustment_type }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
...
.session-date,.item-head p,.item-note{margin:0;color:var(--text-soft)}
...
.item-badge{background:var(--panel-soft);color:var(--text);flex-shrink:0}.item-badge.main{background:rgba(251,191,36,.18);color:#92400e}
.decision-pill.accepted{background:rgba(34,197,94,.16);color:#166534}.decision-pill.guided{background:rgba(59,130,246,.14);color:#1d4ed8}.decision-pill.modified{background:rgba(239,68,68,.12);color:#b91c1c}
...
.record-table-wrap{overflow:auto}.record-table{width:100%;border-collapse:collapse}
